...
public final class Display {
        ...
        synchronized (LOCK) {
            ...
            if (SINGLE_NATIVE_WINDOW_MODE) {
                if (RemixBridgeNative.isAvailable()) {
                    RemixLifecycleBridge.setOutputWindowFullscreen(shouldBeFullscreen);
                }
                return;
            }
            try {
        ...
        synchronized (LOCK) {
            ...
            try {
                ...
                if (SINGLE_NATIVE_WINDOW_MODE) {
                    ...
                    if (RemixBridgeNative.isAvailable() && RemixLifecycleBridge.isOutputCloseRequested()) {
    ...
    public static long resolveNativePresentationWindowHandle() {
        synchronized (LOCK) {
            ...
            if (parent != null) {
                long parentWindow = RemixLifecycleBridge.resolveAwtWindowHandle(parent);
                if (parentWindow != 0L) {
                    return parentWindow;
                }
            }
    ...
    private static void requestSingleNativeInputFocus() {
        ...
        if (presentationWindow != 0L) {
            RemixLifecycleBridge.focusWindow(presentationWindow);
        }
    }
    ...
    private static boolean attachToParentCanvas(DisplayMode mode) throws Exception {
        ...
        if (!RemixLifecycleBridge.embedCompatibilityWindow(childWindow, parentWindow, mode.getWidth(), mode.getHeight())) {
            return false;
        }
    ...
    private static void requestEmbeddedInputFocus(boolean force) {
        ...
        boolean nativeWindowActive = RemixBridgeNative.isAvailable() && RemixLifecycleBridge.hasNativeWindowFocus();
        ...
        try {
            ...
            if (childWindow != 0L && RemixLifecycleBridge.focusWindow(childWindow)) {
                active = true;
            }
        } catch (Exception exception) {
    ...
    private static boolean currentActiveState() {
        ...
        try {
            ...
            return RemixLifecycleBridge.isEmbeddedWindowActive(embeddedChildWindowHandle, embeddedParentWindowHandle);
        } catch (Exception exception) {
    ...
    private static void syncSingleNativeWindowSize() {
        ...
        int width = RemixLifecycleBridge.getOutputWindowWidth();
        int height = RemixLifecycleBridge.getOutputWindowHeight();
        if (width <= 0 || height <= 0) {
            return;
        }
