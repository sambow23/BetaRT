 ...
 * Site names are interned once via {@link RemixBridgeNative#nRegisterPerfSite},
 * so every subsequent sample stores a dense {@code int} id and avoids JNI
 * string marshalling. Buffers flush in batch through
 * {@link RemixBridgeNative#nRecordJavaSampleBatch} either when they fill up or
...
public final class HookProfiler {
    ...
    private static final class ThreadBuffer {
        ...
        private void flushLocked() {
            if (count == 0) return;
            try {
                RemixBridgeNative.nRecordJavaSampleBatch(ids, nanos, count);
            } catch (Throwable t) {
    ...
    private static int resolveSiteId(int side, String site) {
        ...
        int id;
        try {
            id = RemixBridgeNative.nRegisterPerfSite(side, site);
        } catch (Throwable t) {
    ...
    public static void recordCount(String site, long count) {
        if (!ENABLED) return;
        try {
            RemixBridgeNative.nRecordJavaCount(SIDE_HOOK, site, count);
        } catch (Throwable t) {
